...
function start_tempest_tests {
  ls -la
  ls -la /etc
  cat /octavia-tempest-etc/tempest_accounts.yaml
  echo -e "\n === PRE-CONFIG STEP  === \n"
  ...
  rally --debug verify create-verifier --type tempest --name {{ .Chart.Name }}-verifier --system-wide --source https://github.com/sapcc/tempest --version ccloud-python3
  RALLY_EXIT_CODE=$(($RALLY_EXIT_CODE + $?))
  ...
  rally --debug verify start --serial --detailed --pattern '{{ required "Missing run_pattern value!" .Values.run_pattern }}' --skip-list /{{ .Chart.Name }}-etc/tempest_skip_list.yaml --xfail-list /{{ .Chart.Name }}-etc/tempest_expected_failures_list.yaml
  RALLY_EXIT_CODE=$(($RALLY_EXIT_CODE + $?))
